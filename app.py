...
with tab1:
    ...
    with col_in:
        tickers_input = st.text_input(
            ...
            help="Supports stocks, ETFs, crypto (BTC-USD), indices (^GSPC)"
        )
    with col_hint:
    ...
    run_corr = st.button("🚀 Run Correlation Analysis", key="run_corr")
    if run_corr:
        if not tickers_input.strip():
            ...
            if len(tickers) < 2:
                st.error("❌ Need at least 2 tickers for correlation analysis.")
            else:
