...
def _plot_return_by_confidence(df: pd.DataFrame):
    ...
    bars  = axes[0].bar(confs, [a or 0 for a in accs], color=colors, edgecolor=BORDER, linewidth=0.4)
    axes[0].axhline(50, color=MUTED, linewidth=1, linestyle="--")
    axes[0].set_ylim(0, 100)
    axes[0].set_title("Accuracy by Confidence", color=CYAN, fontsize=9)
    axes[0].set_ylabel("Accuracy %", color=MUTED, fontsize=8)
    for bar, a in zip(bars, accs):
        if a: axes[0].text(bar.get_x()+bar.get_width()/2, (a or 0)+1,
    ...
    axes[1].bar(confs, rets, color=rc, edgecolor=BORDER, linewidth=0.4)
    axes[1].axhline(0, color=MUTED, linewidth=0.8)
    axes[1].set_title("Avg Return % by Confidence", color=CYAN, fontsize=9)
    axes[1].set_ylabel("Return %", color=MUTED, fontsize=8)
    fig.tight_layout()
    return fig
...
def _plot_strategy_ranking(ranked: pd.DataFrame):
    ...
    bars = ax.barh(ranked["Signal Type"][::-1], ranked["Composite_Score"][::-1],
...
def _plot_equity_curves_by_type(df: pd.DataFrame):
    ...
    for i, cat in enumerate(df_sorted["signal_category"].unique()):
        ...
        ax.plot(pd.to_datetime(sub["date"]), (sub["cum_ret"] - 1) * 100,
                color=palette[i % len(palette)], linewidth=1.5, label=cat, alpha=0.9)
    ...
    ax.legend(fontsize=8, facecolor=CARD, edgecolor=BORDER, labelcolor=TEXT, loc="upper left", ncol=2)
...
def run_signal_profitability():
    ...
    data = {"df_eval": df_eval, "prof_df": prof_df, "ranked": ranked, "vol_map": vol_map, "n": n}
...
def _display(data: dict):
    ...
    if not ranked.empty:
        ...
        disp = ranked.rename(columns={
            "Total_Signals": "Signals", "Avg_Accuracy": "Accuracy %",
            "Avg_Return": "Avg Return %", "Avg_Vol_Adj": "Vol-Adj Return",
    ...
    if not ranked.empty:
        ...
        if len(hc)>=3 and len(lc)>=3:
            ...
            if hc_ret > lc_ret + 1:
                insights.append(f"✅ **Confidence scoring is working**: HIGH confidence signals return {hc_ret:+.1f}% vs {lc_ret:+.1f}% for LOW confidence.")
            else:
                insights.append(f"⚠️ **Confidence scoring may need re-calibration**: HIGH ({hc_ret:+.1f}%) is not significantly better than LOW ({lc_ret:+.1f}%).")
    ...
    <div style="margin-top:1rem;padding:0.8rem;border:1px solid #1e3a5f;border-radius:8px;
        ...
        ⚠️ Past signal performance does not guarantee future returns. Not financial advice.
    </div>
    """, unsafe_allow_html=True)
